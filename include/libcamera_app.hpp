        ...
        if (!camera_manager_) {
            ...
            if (ret)
                throw std::runtime_error(
                    "camera manager failed to start,"
                    "code " +
                    std::to_string(-ret)
                );
        }
...
struct FrameInfo {
    FrameInfo(libcamera::ControlList &ctrls)
        ...
        if (ae) aelock = (*ae == libcamera::controls::AeStateSearching);
    }
...
private:
    // Info text tokens.
    inline static const std::string tokens[] = {"%frame", "%fps", "%exp", "%ag", "%dg", "%rg", "%bg", "%focus", "%aelock"};
};
